...
local function ReadKeyFile(name)
    if not isfile or not isfile(name) then
        return nil
    end
...
local function GetSavedKey()
    return ReadKeyFile("verified_key.txt")
        or ReadKeyFile(KEY_FILE)
        or NormalizeKey(Config.Key)
end

local function SaveKey(key)
    key = NormalizeKey(key)

    if not key or key == "KEYLESS" then
        return
    end
...
local function InjectSavedKey()
    local key = GetSavedKey()
    ...
    pcall(function()
        if writefile
            and (not isfile or not isfile("verified_key.txt")) then
...
local function CaptureVerifiedKey()
    if not getgenv then
        return
    end

    local key =
        NormalizeKey(getgenv().SCRIPT_KEY)

    if key and key ~= "KEYLESS" then
        SaveKey(key)
    end
end
